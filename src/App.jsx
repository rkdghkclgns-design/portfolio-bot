...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch('/api/prompts/interview-basic');
  ...
  const startCrawl = async () => {
    ...
    try {
      ...
      const evtSource = new EventSource('/api/crawl/stream');
      ...
      evtSource.onerror = () => {
        ...
        fetch('/api/crawl/status').then(r => r.json()).then(status => {
          if (status.running) {
            // 아직 진행 중이면 잠시 후 재연결
            setTimeout(() => {
              const retry = new EventSource('/api/crawl/stream');
            ...
            if (!isErr) {
              fetch('/api/jobs').then(r => r.ok ? r.json() : null).then(jobs => { if (jobs) setJobs(jobs); });
            }
          }
        }).catch(() => {
  ...
  // ── 렌더링 ────────────────────────────────────────────────────────────
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100 relative">

      {/* ── Sidebar ─────────────────────────────────────────────────── */}
      <div className="w-64 bg-slate-900 text-white flex flex-col shadow-xl z-10 shrink-0">
        {/* 로고 */}
        <div className="p-6 flex items-center gap-3 border-b border-slate-800">
          <div className="bg-indigo-500 p-2 rounded-lg"><Gamepad2 size={24} className="text-white" /></div>
          <div>
      ...
      <div className="flex-1 overflow-auto bg-slate-50 p-8 custom-scrollbar">
        <div className="max-w-4xl mx-auto pb-20">
          ...
          {/* 에러 / 정보 메시지 */}
          {error && (
            <div className="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-md flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
              <AlertCircle className="text-red-500 mt-0.5 shrink-0" size={20} />
              <p className="text-red-700 text-sm whitespace-pre-line">{error}</p>
            </div>
          )}
          {infoMessage && (
            <div className="mb-6 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-md flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
